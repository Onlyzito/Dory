...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (member.roles.highest.position >= interaction.member.roles.highest.position)
            return interaction.reply({ content: '**O cargo do usuário é mais alto que o seu! Portanto, você não consegue expulsar ele.**', ephemeral: true })

        if (interaction.guild.me.roles.highest.position <= member.roles.highest.position)
            return interaction.reply({ content: '**O cargo do usuário é mais alto que o meu! Portanto, não consigo expulsar ele.**', ephemeral: true })
